...
window.addEventListener('scroll', () => {
    ...
    // Mostra banner após rolar 300px
    if (scrollTop > 300) {
...
// E-book CTA - Tracking de conversão
const ebookCTA = document.getElementById('ebookCTA');
if (ebookCTA) {
    ebookCTA.addEventListener('click', () => {
        // Track evento no Google Analytics
        if (typeof gtag !== 'undefined') {
            gtag('event', 'click_checkout_ebook', {
                'event_category': 'E-book',
                'event_label': 'Guia NR-1',
                'value': 29.90,
                'currency': 'BRL'
            });
        }
        
        // Track evento no Facebook Pixel
        if (typeof fbq !== 'undefined') {
            fbq('track', 'InitiateCheckout', {
                value: 29.90,
                currency: 'BRL',
                content_name: 'Guia Completo NR-1',
                content_type: 'product'
            });
        }
        
        console.log('Redirecionando para checkout Kiwify...');
    });
}